...
app.post('/add-friend', function(req, resp) {
    if (req.session.user) {
        db.query('INSERT INTO friends (friendly_user, befriend_with) VALUES ($1, $2) RETURNING fid, befriend_with', [req.session.user.username, req.body.username], function(err, result) {
            if (err) {
                console.log(err);
                resp.send({status: 'error'});
            } else if (result !== undefined && result.rowCount === 1) {
                let message = `<p><i><b>*** This message is sent by the system on behalf of the requesting user. ***</i></b></p><p>If you would like to accept this friend request, click on the link below.</p><p><a href='/accept-friend-request?id=${result.rows[0].fid}'>Accept Friend Request</a></p>`;

                db.query("INSERT INTO messages (sender, recipient, subject, message) VALUES ($1, $2, 'You have a friend request from " + req.session.user.username + "', $3)", [req.session.user.username, req.body.username, message], function(err, result) {
                    if (err) {
                        console.log(err);
                        resp.send({status: 'error'});
                    } else if (result !== undefined && result.rowCount === 1) {
...
app.post('/delete-friend', function(req, resp) {
    if (req.session.user) {
        ...
        db.query('DELETE FROM friends WHERE friendly_user = $1 AND befriend_with = $2 RETURNING befriend_with', [req.session.user.username, req.body.username], (err, result) => {
            console.log(result);
            if (err) {
                console.log(err);
                resp.send({status: 'error'});
