...
app.post('/get-replies', function(req, resp) {
    db.query('SELECT SUM(count_replies) AS num_of_replies FROM (SELECT COUNT(reply_to_post_id) AS count_replies FROM posts WHERE belongs_to_post_id = $1 AND reply_to_post_id IS NOT NULL GROUP BY reply_to_post_id) AS p2;', [req.body.post_id], function(err, result) {
        if (err) { console.log(err); }
...
app.post('/get-post-details', function(req, resp) {
    if (req.session.user) {
        if (req.session.user.privilege > 1) {
            ...
            db.query(queryString, querySearchParams, function(err, result) {
                if (err) {
                    console.log(err);
                    resp.send({status: 'error'});
                } else if (result !== undefined) {
...
app.get('/get-num-of-posts-in/:subtopic/:page', function(req, resp) {
    ...
    db.query('SELECT COUNT(post_id) AS total_posts FROM posts LEFT JOIN subtopics ON subtopics.subtopic_id = posts.post_topic WHERE subtopics.subtopic_title = $1 AND posts.belongs_to_post_id IS NULL', [fn.capitalize(subtopic).replace('_', ' ')], function(err, result) {
        if (err) {
            console.log(err);
            resp.send({status: 'error'});
